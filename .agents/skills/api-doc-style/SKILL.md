...
- 所有参数表、响应表、字段名、取值标签和代码样式选项框都必须完整显示，禁止出现截断、省略号、被容器裁剪或只显示半个字符的情况。
- 不得对表格单元格或选项框使用 `text-overflow: ellipsis`、`line-clamp`、隐藏溢出等会丢失内容的处理。长字段名、路径和取值应优先通过合理扩大列宽或表格最小宽度解决。
- 表格内容过宽时，在表格所在的 `__content` 容器中启用 `overflow-x: auto` 横向滚动；滚动区域应局部作用于表格，不得导致整页被横向撑开。
- 普通说明和取值文本可以自然换行，紧凑布局下尽量控制在三行内；超过三行时应继续完整显示或扩大可滚动表格宽度，不得通过隐藏或截断压缩到三行。
- 字段名、路径和代码选项不得被强制按任意字符拆分，避免出现单独的括号、句点或半个标识符。只有在扩大列宽和横向滚动仍无法避免溢出时，才允许使用可读的安全断行。
- 验证页面时必须同时检查桌面和窄屏布局：确认可见文本与源文档一致，选项框没有 `scrollWidth > clientWidth` 的隐藏内容；如果表格本身有意宽于视口，则确认其滚动容器存在且可以横向移动。
